...
const lbl = { fontSize: 12, color: "#8b949e", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 };
const mn = (s, c = "#e6edf3") => ({ fontSize: s + 2, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: c });
const pS = { fontSize: 16, lineHeight: 1.8, color: "#c9d1d9", margin: "0 0 8px" };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#0a0e17", color: "#e6edf3", fontFamily: "'Instrument Sans',sans-serif", padding: "20px 14px" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 14 }}>
          <h1 style={{ fontSize: 36, fontWeight: 700, margin: "0 0 6px", background: "linear-gradient(135deg,#e6edf3,#00ff87)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>⚡ Stock Pulse</h1>
          <p style={{ color: "#e6edf3", fontSize: 17, fontWeight: 600, margin: "0 0 4px" }}>Invierte como un pro. Sin serlo.</p>
          <p style={{ color: "#6e7681", fontSize: 13, margin: 0 }}>Análisis en profundidad · Mercado global · Decide con información</p>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 14, flexWrap: "wrap", marginBottom: 14, padding: "5px 12px", backgroundColor: clock.o ? "rgba(0,255,135,.04)" : "rgba(136,198,255,.04)", border: `1px solid ${clock.o ? "rgba(0,255,135,.1)" : "rgba(136,198,255,.1)"}`, borderRadius: 8 }}>
          <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
            ...
            <span style={{ fontSize: 14, fontFamily: "'JetBrains Mono',monospace", color: clock.o ? "#00ff87" : "#88c6ff" }}>🇺🇸 NYSE · {clock.t}</span>
          </span>
          <span style={{ fontSize: 13, fontFamily: "'JetBrains Mono',monospace", color: "#6e7681" }}>🇪🇸 {clock.m} · 🇺🇸 {clock.n}</span>
          {stats.tot > 0 && <span style={{ fontSize: 13, fontFamily: "'JetBrains Mono',monospace", color: "#6e7681" }}>📊 {stats.tot} pred · {stats.wr}% win</span>}
        </div>
        ...
        <div style={{ display: "flex", marginBottom: 18, borderBottom: "1px solid #1e2433" }}>
          {[["analyze", "🌍 Analizar"], ["monitor", `📡 Monitor${stats.pend ? ` (${stats.pend})` : ""}`], ["history", "📊 Historial"], ["profile", "👤 Perfil"]].map(([k, l]) => (
            <button key={k} onClick={() => setTab(k)} style={{ flex: 1, padding: "12px 0", fontSize: 14, fontWeight: 600, fontFamily: "inherit", backgroundColor: "transparent", color: tab === k ? "#00ff87" : "#6e7681", border: "none", borderBottom: tab === k ? "2px solid #00ff87" : "2px solid transparent", cursor: "pointer" }}>{l}</button>
          ))}
        </div>
